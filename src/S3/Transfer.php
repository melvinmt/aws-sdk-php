...
class Transfer implements PromisorInterface
{
     ...
     *   string $dest, AwsSDK\CommandInterface $command. The provided command will
    ...
    private function createDownloadPromise()
    {
        ...
        return (new AwsSDK\CommandPool($this->client, $commands, [
    ...
    private function createUploadPromise()
    {
        ...
        $files = \AwsSDK\map($this->getUploadsIterator(), function ($file) {
    ...
    private function getUploadsIterator()
    {
        if (is_string($this->source)) {
            return AwsSDK\filter(
                AwsSDK\recursive_dir_iterator($this->sourceMetadata['path']),
    ...
    private function getDownloadsIterator()
    {
        if (is_string($this->source)) {
            ...
            $files = $this->client
                ...
                ->search('Contents[].Key');
            $files = AwsSDK\map($files, function ($key) use ($listArgs) {
                return "s3://{$listArgs['Bucket']}/$key";
            });
            return AwsSDK\filter($files, function ($key) {
